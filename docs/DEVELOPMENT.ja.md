...
| `overlay.rs` | NSPanel 変換、ウィンドウレベル・collection behavior、配置、コンテンツ追従リサイズ (ウィンドウが pill に密着するため pill 外のクリックは下のアプリに届く) |
...
| `APPLE_API_ISSUER` / `APPLE_API_KEY` / `APPLE_API_KEY_BASE64` | 公証 (推奨): App Store Connect API キーの Issuer ID・Key ID・`.p8` の base64 |
| `APPLE_ID` / `APPLE_PASSWORD` / `APPLE_TEAM_ID` | 公証 (代替): Apple ID・アプリ用パスワード・チーム ID |
