...
- **議事録エクスポート** — セッションの発話履歴を Markdown で `~/Downloads` に書き出し。AI による要点・決定事項・TODO のサマリ付きも可能。1〜2 時間超の長い会議も分割要約に対応し、キャプチャ停止時の自動書き出しも設定可能
- **セッション履歴** — キャプチャごとの履歴をローカルに保存。過去の会議も後から設定画面で選んで議事録を書き出せます
...
- オーバーレイは**ドラッグ**で移動、**ホバー**で操作ボタン(キャプチャ切替・クリックスルー・設定・縮小)が出現
- マウスをブロックするのは見えている字幕 pill の部分だけ。オーバーレイの透明な余白へのクリックは下のアプリにそのまま届きます。**縮小**(−)で pill を小さなチップに折りたたみ可能(キャプチャは継続)
- **クリックスルー**を有効にすると字幕越しに下のアプリを操作可能(解除はメニューバーから)
- **議事録の書き出し**は Markdown を `~/Downloads` に保存。「サマリ付き」は Ollama が要点・決定事項・TODO を生成。設定画面では**停止時の自動書き出し**の有効化や、**過去のセッション**一覧からの再書き出しも可能
